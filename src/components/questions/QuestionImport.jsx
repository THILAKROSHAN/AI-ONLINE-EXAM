...
import { importQuestionsFromExcel } from '../../services/api/question';
import Button from '../common/Buttons/Button';
import Spinner from '../common/Loading/Spinner';
import Toast from '../common/Notifications/Toast';

const QuestionImport = ({ onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        setSuccess(`Successfully imported ${result.data.count || 0} questions`);
        setTimeout(() => {
          onSuccess();
          onClose();
        }, 2000);
  ...
  const downloadTemplate = () => {
    const headers = [
      'Question Text', 'Type', 'Difficulty', 'Option 1', 'Option 2',
      'Option 3', 'Option 4', 'Correct Answer', 'Explanation',
      'Subject', 'Topic', 'Marks', 'Negative Marks', 'Tags'
    ];
    
    const csvContent = headers.join(',') + '\n' +
      '"Sample question?","mcq","medium","Option A","Option B","Option C","Option D","Option A","Sample explanation","Mathematics","Algebra","1","0","tag1,tag2"';
    ...
    <form onSubmit={handleSubmit} className="p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">
          Bulk import questions from an Excel or CSV file
        </p>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-between">
          ...
          <div className="space-x-3">
            ...
            >
              {loading ? <Spinner size="sm" /> : 'Import Questions'}
            </Button>
          </div>
        </div>
      </div>
